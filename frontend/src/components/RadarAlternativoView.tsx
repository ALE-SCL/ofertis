    ...
    <div className="space-y-6 animate-fadeIn">
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
          <div className="rounded-2xl border border-stone-200/80 bg-white p-4 sm:p-5 shadow-xs">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold text-stone-500 uppercase tracking-wider">Ahorro Promedio</span>
              <span className="rounded-xl bg-orange-50 p-2 text-orange-600">
                <Percent className="h-4 w-4" />
              </span>
            </div>
            <div className="mt-2 flex items-baseline gap-2">
              <span className="text-2xl font-black text-stone-900">{kpis.avg_savings_pct}%</span>
              <span className="text-xs font-semibold text-orange-600">vs Retail</span>
            </div>
          </div>

          <div className="rounded-2xl border border-stone-200/80 bg-white p-4 sm:p-5 shadow-xs">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold text-stone-500 uppercase tracking-wider">Máximo Ahorro</span>
              <span className="rounded-xl bg-amber-50 p-2 text-amber-600">
                <Flame className="h-4 w-4" />
              </span>
            </div>
            <div className="mt-2 flex items-baseline gap-2">
              <span className="text-2xl font-black text-stone-900">{kpis.max_savings_pct}%</span>
              <span className="text-xs font-semibold text-amber-600">Lo Valledor</span>
            </div>
          </div>

          <div className="rounded-2xl border border-stone-200/80 bg-white p-4 sm:p-5 shadow-xs">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold text-stone-500 uppercase tracking-wider">Super Ahorros</span>
              <span className="rounded-xl bg-orange-50 p-2 text-orange-600">
                <Award className="h-4 w-4" />
              </span>
            </div>
            <div className="mt-2 flex items-baseline gap-2">
              <span className="text-2xl font-black text-stone-900">{kpis.super_deals_count}</span>
              <span className="text-xs font-semibold text-orange-600">&gt; 25% ahorro</span>
            </div>
          </div>

          <div className="rounded-2xl border border-stone-200/80 bg-white p-4 sm:p-5 shadow-xs">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold text-stone-500 uppercase tracking-wider">Canales Monitoreados</span>
              <span className="rounded-xl bg-amber-50 p-2 text-amber-700">
                <Store className="h-4 w-4" />
              </span>
            </div>
            <div className="mt-2 flex items-baseline gap-2">
              <span className="text-2xl font-black text-stone-900">{kpis.monitored_stores_count}</span>
              <span className="text-xs font-semibold text-amber-700">Tiendas / Hubs</span>
            </div>
          </div>
        </div>
      )}
          ...
          <button
            ...
            className={`px-3.5 py-1.5 rounded-xl text-xs font-bold whitespace-nowrap transition-all shadow-xs ${
              selectedStore === 'todas'
                ...
                : 'bg-white hover:bg-orange-50 text-stone-700 border border-stone-200 hover:border-orange-200'
            }`}
          >
      ...
      <div>
        ...
        {loading ? (
          <div className="py-20 text-center bg-white rounded-3xl border border-stone-200">
            ...
            <p className="text-sm font-bold text-stone-700">Consultando precios en canales alternativos...</p>
            <p className="text-xs text-stone-500 mt-1">Calculando brechas contra el retail tradicional</p>
          </div>
          ...
          <div className="py-16 text-center bg-white rounded-3xl border border-stone-200 p-8 space-y-3">
            ...
            <h3 className="text-base font-bold text-stone-800">No encontramos oportunidades para tu búsqueda</h3>
      ...
      <div className="mt-10 pt-8 border-t border-stone-200">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {stores.map((store) => (
            <div key={store.id} className="rounded-2xl border border-stone-200/80 bg-white p-4 shadow-xs hover:border-orange-300 transition-colors flex flex-col justify-between">
              <div>
                <div className="flex items-start justify-between">
                  <span className="inline-block rounded-md bg-orange-50 px-2 py-0.5 text-[10px] font-bold text-orange-800 border border-orange-200">
                    {store.type_label}
                  </span>
                  <a 
                    href={store.website} 
                    ...
                    className="rounded-lg p-1 text-stone-400 hover:bg-stone-100 hover:text-orange-600"
                    title="Visitar sitio oficial"
                  >
                    <ExternalLink className="h-3.5 w-3.5" />
                  </a>
                </div>
                <h4 className="mt-2 text-sm font-bold text-stone-900">{store.name}</h4>
                <p className="mt-1 text-xs text-stone-600 leading-relaxed">{store.description}</p>
              </div>
              <div className="mt-3 flex items-center gap-1 text-[11px] font-semibold text-orange-900 bg-orange-50/70 rounded-md p-1.5 border border-orange-100">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-stone-900/50 backdrop-blur-sm animate-fadeIn">
          <div className="bg-white rounded-3xl max-w-lg w-full p-6 shadow-2xl border border-stone-200 relative">
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3 p-4 bg-stone-50 rounded-2xl border border-stone-100">
                <div>
                  <span className="text-[11px] font-bold text-stone-500 uppercase block">Canal Alternativo</span>
                  <span className="text-xs font-semibold text-orange-700 block">{selectedComparisonOpp.store_name}</span>
                  ...
                  <span className="text-[10px] text-stone-500">({selectedComparisonOpp.unit})</span>
                </div>

                <div className="border-l border-stone-200 pl-3">
                  <span className="text-[11px] font-bold text-stone-500 uppercase block">Benchmark Retail</span>
                  <span className="text-xs font-semibold text-stone-600 block">{selectedComparisonOpp.benchmark_label}</span>
                  <span className="text-xl font-black text-stone-400 line-through mt-1 block">
                    ${Math.round(selectedComparisonOpp.traditional_benchmark_price).toLocaleString('es-CL')}
                  </span>
                  <span className="text-[10px] text-stone-500">4 Supermercados</span>
                </div>
              </div>

              <div className="bg-orange-50 border border-orange-200 rounded-2xl p-4 flex items-center justify-between">
                <span className="text-xs font-bold text-orange-950">Ahorro total estimado:</span>
              ...
              <p className="text-xs text-stone-600 leading-relaxed bg-stone-50 p-3 rounded-xl border border-stone-100 italic">
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button
                  onClick={() => setSelectedComparisonOpp(null)}
                  className="px-5 bg-stone-100 hover:bg-stone-200 text-stone-700 font-bold py-3 rounded-xl text-xs"
                >
                  Cerrar
                </button>
              </div>
