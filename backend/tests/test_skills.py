...
class TestOfertisSkills(unittest.TestCase):
    ...
    def test_unit_normalizer_kg_direct(self):
        skill = UnitNormalizerSkill()
        # 1.5 kg de carne a $12.000 CLP -> $8.000 CLP / kg
        res = self.loop.run_until_complete(
            skill.execute(text="Lomo Liso Vacuno 1.5 kg", price=Decimal("12000"), category="carne_vacuno")
        )
        self.assertEqual(res["package_quantity"], Decimal("1.500"))
        self.assertEqual(res["package_unit"], "kg")
        self.assertEqual(res["unit_price_normalized"], Decimal("8000"))
